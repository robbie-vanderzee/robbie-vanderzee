class TextScramble {
  ...
  setText(newText) {
    ...
    const range = 38.2;
    const temporal = 1.618;
    this.queue = [];
    for (let i = 0; i < length; i++) {
      const from = oldText[i] || '';
      const to = newText[i] || '';
      const start = i;
      const center = Math.floor(range * Math.log(start + 2));
      const margin = Math.floor(range * Math.log(start + 3)) - center;
      const end = temporal * Math.floor(center) + Math.floor(margin * Math.random());
      this.queue.push({
        from,
        to,
        start,
...
function followers() {
  const followers = document.querySelectorAll('[data-follows-y]');
  const owners = new Map();

  followers.forEach(follower => {
    const ownerId = follower.getAttribute('data-follows-y');
    const owner = document.getElementById(ownerId);

    if (owner) {
      if (!owners.has(owner)) {
        owners.set(owner, new Set());
      }
      owners.get(owner).add(follower);
    }
  });

  function update(follower, top) {
  ...
  const observer = new ResizeObserver((entries) => {
    entries.forEach(entry => {
      ...
      owners.get(target).forEach(follower => update(follower, top));
    });
  });

  owners.forEach((followers, followable) => {
    const top = followable.getBoundingClientRect().top + window.scrollY;
    followers.forEach(follower => update(follower, top));
    observer.observe(followable);
  })
}
...
window.onload = function (window, event) {
  followers();
  scramblers();
};
